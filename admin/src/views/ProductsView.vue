<template>
  <div>
        ...
        <Card>
          <template #content>
            <div class="flex flex-column gap-4">
              ...
              <div>
                <div class="filter-section-label">Vendor</div>
                <Dropdown
          ...
          <div v-for="product in productStore.products" :key="product.id" class="col-12 sm:col-6 md:col-4 xl:col-3">
            <div class="product-card">
              ...
              <div class="p-3 flex-1 flex flex-column">
                ...
                <div class="product-desc mb-3">{{ productDescription(product) }}</div>
    ...
    <Dialog v-model:visible="dialogVisible" :header="isEditing ? 'Edit Product' : 'New Product'" :style="{ width: '30rem' }" modal>
      <div class="flex flex-column gap-3">
        <div class="flex flex-column gap-2">
          <label for="title">Title</label>
          <InputText id="title" v-model="productForm.title" :class="{ 'p-invalid': formSubmitted && !productForm.title }" />
        </div>
        <div class="flex flex-column gap-2">
          <label for="vendor">Vendor</label>
          <InputText id="vendor" v-model="productForm.vendor" />
        </div>
        <div class="flex flex-column gap-2">
          <label for="productType">Type</label>
          <InputText id="productType" v-model="productForm.product_type" />
        </div>
        <div class="flex flex-column gap-2">
          <label for="status">Status</label>
          <Dropdown id="status" v-model="productForm.status" :options="statusOptions" option-label="label" option-value="value" />
        </div>
      </div>
      <template #footer>
        <Button label="Cancel" icon="pi pi-times" text @click="closeDialog" />
        <Button label="Save" icon="pi pi-check" :loading="saving" @click="saveProduct" />
      </template>
    </Dialog>
  </div>
</template>
...
async function saveProduct() {
  ...
  try {
    ...
    if (isEditing.value) {
      await productStore.updateProduct(productForm.id, payload);
      ...
      await productStore.createProduct({
        ...
        handle: productForm.title.toLowerCase().replace(/\s+/g, '-'),
        ...payload
      });
